...
Destructive restores use a two-step Components V2 flow: first pick
which parts of the backup to restore (roles/categories/channels,
toggleable), then a second confirmation screen with green/red buttons
before anything is actually wiped. Merge mode skips straight to a
plain confirmation, since merge never deletes anything."""
...
class RestoreSelectView(discord.ui.LayoutView):
    """Step 1 for a destructive restore - Components V2 card: toggle
    which parts of the backup to restore (roles/categories/channels),
    then Continue. Only whoever ran the command can use these buttons."""

    def __init__(self, author_id: int, backup_id: int, snapshot: dict):
    ...
    def _build(self) -> None:
        ...
        container = discord.ui.Container(
            discord.ui.TextDisplay(f"# Restore backup #{self.backup_id} (destructive)"),
    ...
    def _make_toggle(self, key: str, button: discord.ui.Button):
        async def callback(interaction: discord.Interaction) -> None:
            if interaction.user.id != self.author_id:
                await interaction.response.send_message("This isn't your restore to configure.", ephemeral=True)
                return
    ...
    async def _on_continue(self, interaction: discord.Interaction) -> None:
        ...
        confirm_view = ConfirmDestructiveView(self.author_id)
        ...
        try:
            await interaction.followup.send(
                embed=discord.Embed(title=f"Restored backup #{self.backup_id}", description=summary)
            )
...
class Backup(commands.Cog):
    ...
    async def backup_restore(self, ctx: commands.Context, backup_id: int, mode: str = "merge"):
        ...
        if mode == "destructive":
            view = RestoreSelectView(ctx.author.id, backup_id, snapshot)
